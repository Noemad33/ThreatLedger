...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .hunt-sources { margin: 1rem 0 0; padding-top: 0.75rem; border-top: 1px solid var(--line); font-size: 0.8rem; line-height: 1.7; color: var(--ink-faint); } .hunt-sources b { font-family: "IBM Plex Mono", ui-monospace, monospace; font-size: 0.68rem; letter-spacing: 0.08em; text-transform: uppercase; font-weight: 500; margin-right: 0.35rem; } .hunt-sources a { color: var(--ink-soft); text-decoration: none; border-bottom: 1px solid var(--line); } .hunt-sources a:hover { color: var(--accent); border-color: var(--accent-soft); }
...
<div class="page">
  ...
  <section class="brief-section">
    ...
    <!-- <div class="hunt-card">
      ...
      <div class="hunt-sources"><b>Sources</b> <a href="[real URL of the primary report/IOC file this card is built from]" target="_blank" rel="noopener">[Publisher — report title]</a> · <a href="[real URL of any published rule this card adapted — omit if none]" target="_blank" rel="noopener">[org/repo — filename]</a> · <a href="https://attack.mitre.org/techniques/[Txxxx or Txxxx/xxx]/" target="_blank" rel="noopener">MITRE [Txxxx.xxx]</a> ... one link per MITRE technique used in the card ...</div>
    </div> -->
...
- **Never fabricate a field name.** Use real field/table names for whichever platform(s) apply to the finding:
  - **CrowdStrike Falcon Advanced Event Search** — LogScale/CQL syntax: `event_simpleName=<EventName>` as the base filter (common ones: `ProcessRollup2` for process execution, `DnsRequest` for DNS, `NetworkConnectIP4` for outbound connections), piped `|` filters, and real field names like `ComputerName`, `UserName`, `FileName`, `CommandLine`, `ParentBaseFileName`, `DomainName`, `RemoteAddressIP4`, `TargetFileName`.
    **CrowdStrike Query Language (CQL) is NOT SQL and NOT Splunk SPL — get the syntax right or the console rejects the query outright (a real production failure: `IN (...)` produced "Expected an expression" in the Falcon console and cost the reader time on a live hunt).** Rules:
    - **Match several values with the `in()` function or with `or` — never SQL-style `IN (...)`.** Correct: `| in(field=FileName, values=["a.exe","b.dll"], ignoreCase=true)`; also correct: `| FileName=/^(a\.exe|b\.dll)$/i` or `| FileName=a.exe or FileName=b.dll`. WRONG (invalid, will not parse): `FileName IN ("a.exe","b.dll")`, `event_simpleName IN (DriverLoad, ProcessRollup2)`. To match either of two event types, write `(event_simpleName=DriverLoad or event_simpleName=ProcessRollup2)` (parentheses required — `or` binds tighter than the implicit AND between filters, so an unparenthesized `or` can silently change what the query means).
    - **Negate with `not` / `!=` / `!in(...)`**, not `NOT IN`.
    - **There is no `append [search ...]`, `union`, `join [ ... ]` sub-search, or `| search` command — those are Splunk/KQL.** Do not try to combine two different event types with different filters into one query. Instead write **separate, standalone queries** (one `query-pre` block each, with a short `query-label` line above the second one saying what it hunts), or use a single `(event_simpleName=A or event_simpleName=B)` filter when the same field filter applies to both.
    - **Filter syntax:** `field=value`, `field=*wildcard*`, regex `field=/pattern/i` (regex delimited by slashes, `/i` for case-insensitive), joined with `and` / `or` / `not` and parentheses. Pipe stages use `| table([field1, field2])`, `| groupBy([field], function=count())`, `| sort(field)`, `| tail(50)` — square brackets around field lists, function-call style.
    - **Only use `event_simpleName` values you are confident are real** (`ProcessRollup2`, `DnsRequest`, `NetworkConnectIP4`, `NetworkConnectIP6`, `DriverLoad`, `NewExecutableWritten`, `PeFileWritten`, `SyntheticProcessRollup2`, `UserLogon`, `AsepValueUpdate`). Do NOT invent event names (e.g. `FileWritten` does not exist as a Falcon event) — if no verified event covers the behavior, drop the CrowdStrike panel for that behavior and say so in one sentence instead of guessing. A DLL is not a process: hunting a dropped/sideloaded DLL by name in `ProcessRollup2` will never match; use a file-write event (`NewExecutableWritten` / `PeFileWritten`, field `TargetFileName`) or say honestly that the DLL isn't hunted.
    - **Before writing the query, re-read it once as a CQL parser would:** any `IN (`, `append`, `union`, `search event_simpleName`, `stats count by`, or `index=` means you wrote SPL/SQL — rewrite it.
...
- **Every card ends with a Sources line** (the `.hunt-sources` div at the bottom of the card template above) so the reader can jump from a query straight to where it came from. Include, in this order: (1) the primary report or raw IOC file the card is built from, (2) any published rule the card adapted (the same github.com link as in the 'Adapted from a published rule' credit), and (3) one link per MITRE technique shown in the card's chips, as `https://attack.mitre.org/techniques/Txxxx/` (sub-techniques as `Txxxx/xxx/`). Every URL must be one you actually saw work this run — the same no-guessing rule as everywhere else. MITRE URLs are only allowed for technique IDs you verified against attack.mitre.org. If you can't confirm a link, leave it out rather than guess; a shorter Sources line beats a broken or invented one.
